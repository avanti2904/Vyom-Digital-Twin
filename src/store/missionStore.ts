...
interface MissionStore extends MissionState {
  ...
  completeMission: (targetScreen?: AppScreen) => void;
  resumeRealtimeAnalysis: () => void;
...
const initialState: MissionState = {
  ...
  timeMultiplier: 1,   // 1 real second = 1 mission second (1× Real-Time)
  isPaused: false,
  ...
  persist(
    (set, get) => ({
      ...
      tickMission: (realDeltaMs) => {
        ...
        const accel = s.timeMultiplier > 0 ? s.timeMultiplier : 1;
      ...
      completeMission: (targetScreen?: AppScreen) => {
        set((s) => ({
          status: 'completed',
          screen: targetScreen ? targetScreen : 'completion',
          objectiveProgress: 100,
        }));
      },

      resumeRealtimeAnalysis: () => {
        const now = Date.now();
        set({
          status: 'active',
