              ...
              {milestones && milestones.length > 0 ? (
                milestones.map((milestone: any, index: number) => (
                  <li key={index} style={{ 
                    ...
                    <div style={{ fontWeight: 'bold', color: '#fff' }}>{milestone.label || milestone.name || `Milestone ${index + 1}`}</div>
                    <div style={{ fontSize: '0.85em', color: milestone.completed ? '#00ff88' : 'rgba(255,255,255,0.6)' }}>
                      Status: {milestone.completed ? 'COMPLETED' : (milestone.status || 'SCHEDULED')}
                    </div>
                  </li>
                ))
              ) : (
