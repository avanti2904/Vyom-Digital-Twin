...
function tickCrewVitals(crew: CrewMember[], missionDay: number, envRadiation: number): CrewMember[] {
  ...
  return crew.map((c, idx) => {
    ...
    // Impact / decompression / life-support impairment from threats
    if (threatEffects.asteroid || threatEffects.debris) {
      const sev = (threatEffects.asteroid || 0) + (threatEffects.debris || 0);
      targetPressure = Math.max(88.0, 101.3 - sev * 1.8);
      targetSpO2 = Math.max(92.0, 99.2 - sev * 0.8);
    }
    if (threatEffects.power) {
      targetSpO2 = Math.max(93.5, 99.2 - threatEffects.power * 0.6);
      targetPressure = Math.max(94.0, 101.3 - threatEffects.power * 0.5);
    }

    const hr = clamp(lerp(c.heartRateBpm, targetHeartRate, 0.04), 48, 165);
    const spo2 = clamp(lerp(c.spo2Percent, targetSpO2, 0.03), 85, 100);
    ...
    const press = clamp(lerp(c.suitPressureKpa ?? 101.3, targetPressure, 0.04), 80, 104);
    ...
    return {
      ...
      suitPressureKpa: parseFloat(press.toFixed(1)),
...
function tickTelemetry(simDeltaMs: number): Telemetry {
  ...
  // Auto-clear threat effects if store has no active threats
  if (store.activeThreats.length === 0 && Object.keys(threatEffects).length > 0) {
    threatEffects = {};
  }
...
export function cancelAIPipeline(reason = 'Manual Operator Override') {
  ...
  // Clear pipeline tracking so a new pipeline can start
  currentPipelineIncidentId = null;
  currentPipelineThreat = null;
...
function tickObjective() {
  ...
  if ((allDone || timeExpired) && status === 'active') {
    ...
    // Keep user's active screen for continuous real-time analysis
    store.completeMission(store.screen);
  }
}
...
function tickEnvironment() {
  ...
  const threatRad = (threatEffects.radiation || 0) * 7.5 + (threatEffects.solar || 0) * 3.0;
  const effectiveSolar = clamp(solarLevel + (threatEffects.solar || 0) * 0.8, 0, 10);
  const effectiveRad = clamp(10 + solarLevel * 5 + threatRad, 5, 120);

  const classification = effectiveRad > 50 || effectiveSolar > 7 ? 'critical'
    : effectiveRad > 30 || effectiveSolar > 4.5 ? 'warning'
    : effectiveSolar > 2 ? 'normal' : 'low';

  store.setEnvironment({
    solarActivityLevel: effectiveSolar,
    radiationLevel: effectiveRad,
    magneticFieldNT: 31200 + (threatEffects.solar ? threatEffects.solar * 1200 : 0),
    temperatureRangeC: [-90 - Math.round(solarLevel * 2), 120 + Math.round(solarLevel * 5)],
    debrisDensity: 1.2 + (threatEffects.debris || 0) * 2.5 + (threatEffects.asteroid || 0) * 4.0,
    classification,
    dataSource: 'simulation',
  });
}
...
export function stopAndResetSimulation() {
  const store = useMissionStore.getState();
  // Mitigate all active threats
  store.activeThreats.forEach((t) => store.mitigateThreat(t.id));
  clearThreatEffects();
  cancelAIPipeline('Simulation reset to baseline by operator');

  // Restore simState to nominal baseline
  simState.health = 98.5;
  simState.cpuTemp = 42.0;
  simState.payloadTemp = 28.0;
  simState.batteryTemp = 19.5;
  simState.battery = 96.0;
  simState.roll = 0.12;
  simState.pitch = -0.08;
  simState.yaw = 0.04;
  simState.angularVel = 0.01;

  // Restore crew vitals to nominal
  if (store.crew && store.crew.length > 0) {
    const resetCrew = store.crew.map((c) => ({
      ...c,
      heartRateBpm: 72,
      spo2Percent: 99.1,
      respirationBpm: 14,
    ...
    useMissionStore.setState({ crew: resetCrew });
  }
  ...
  store.logEvent({
    id: `ev-sim-reset-${Date.now()}`,
    timestamp: Date.now(),
    missionDay: store.missionDay,
    eventType: 'recovery',
    severity: 'nominal',
    description: 'SIMULATION RESET: Operator stopped active scenario. All subsystems restored to safe baseline.',
