...
/** Default: 1 real second = 1 mission simulation second (1× Real-Time) */
export const DEFAULT_ACCELERATION = 1;
...
class MissionClockEngine {
  ...
  start() {
    ...
    console.log('[MissionClock] Started — timestamp-based mode (1× Real-Time default)');
  }
  ...
  private loop = (frameTs: number) => {
    ...
    // Allow continuous real-time analysis
    const missionDay     = rawMissionDay;
    ...
    // ── Mission nominal completion at planned duration ─────────────────────
    if (rawMissionDay >= totalDays && !this.missionCompleteEmitted) {
      this.missionCompleteEmitted = true;
      console.log(`[MissionClock] Mission reached nominal duration: ${totalDays} days`);
      ...
      useMissionStore.getState().logEvent({
        ...
        eventType: 'milestone',
        severity: 'nominal',
        description: `MISSION NOMINAL OBJECTIVES ACHIEVED — ${store.config?.name ?? 'VYOM Mission'} nominal timeline reached. Real-time telemetry monitoring continuing in extended operations.`,
        source: 'Mission Clock Engine',
        immutable: true,
      });

      // Emit to event bus without hijacking screen
